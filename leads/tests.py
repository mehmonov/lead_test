...
class LeadSubmissionTests(APITestCase):
    def setUp(self):
        ...
        self.valid_payload = {
            ...
            'resume': SimpleUploadedFile("cv.pdf", b"dummy-data", content_type="application/pdf"),
        }
    ...
    def test_create_lead_invalid_resume_type(self):
        payload = self.valid_payload.copy()
        payload['resume'] = SimpleUploadedFile("text.txt", b"invalid-content", content_type="text/plain")
        response = self.client.post(self.url, payload, format='multipart')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
